...
![avatar](https://files.catbox.moe/kquozh.jpg)
![avatar](https://files.catbox.moe/l3qukk.jpg)
...
每两个最小节点生成一个父节点，子节点共有n个，生成n-1个父节点，共2n-1个节点
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？
将类型转换的数值进行调整，小写字母可改为(0x61 + j), 大小写同时编码可依次调用检测大写小写的两个循环，只需调整第二个循环的i的初始值便可。
