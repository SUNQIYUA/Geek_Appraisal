...
int main()
{
	...
	for(int i = 0; i < 27; i++)
    {
        if(arr_big[i] != 0 && count2 < count)
        {
			...
			count2++;	//将数组arr_big中的非零项依次赋进arr_small中以此表示各字母出现次数
        }
    }//统计输入字符串中字母的出现次数
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  //无左右子树节点 
        {
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  //判断是否存在父结点y以及是否为初始结点 
        {
            break;
        }
        if(huffman[i][3] < min1 && huffman[i][0] == -1)  //比较结点大小，判断该结点是否最小 
        {
            min1 = huffman[i][3];
            ii[0] = i;
        }
